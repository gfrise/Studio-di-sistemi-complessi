...
int main(int argc, char* argint[]) {
  ...
  FILE *fp0;
  double param[6];
  fp0=fopen("input_parametersRISK.dat", "r");
  for(i=0; i<6;i++){
     fscanf(fp0,"%lf \n",&param[i]);
  }
  fclose(fp0);
  ...
  seedsh=atoi(argint[2]);
  printf("%d \n",seedsh);


  printf("\n Inizio della simulazione \n");
  double *Z;
  Z=allocamem1(nR*enne);
  
  Z=myrandgauss(nR*enne,seed,0.,2*dt);
  ...
  FILE *fp5;
  fp5=fopen("serienumerica_shuff.dat", "w");
  for(i=0; i<nR;i++){
     fprintf(fp5,"%lf \n",X[i]);
  }
  fclose(fp5);  
  ...
  FILE *fp3;
  fp3=fopen("auto-correlationRISKEN_shuff.dat","w");
